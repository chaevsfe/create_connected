    ...
    protected void collectPieces(BlockState state, List<Piece> pieces) {
        ...
        for (Direction direction : Iterate.directions) {
            ...
            if (direction.getAxis().isVertical()) {
                ...
                pieces.add(piece(0, direction == Direction.UP,
                        vec3(1 - west, 0, 1 - north),
                        aabb(14 + east + west, 1, 14 + north + south).move(1 - west, 0, 1 - north),
                        cull(MutableCullFace.NORTH * (1 - north) | MutableCullFace.SOUTH * (1 - south)
                                | MutableCullFace.EAST * (1 - east) | MutableCullFace.WEST * (1 - west))
                ));
            } else {
                ...
                pieces.add(piece((int) direction.toYRot() + 180, false,
                        vec3(1 - right, 1 - down, 0),
                        aabb(14 + left + right, 14 + up + down, 1).move(1 - right, 1 - down, 0),
                        cull(MutableCullFace.UP * (1 - up) | MutableCullFace.DOWN * (1 - down)
                                | MutableCullFace.EAST * (1 - left) | MutableCullFace.WEST * (1 - right))
                ));
            }
        }
    }
